...
test.describe("form layouts page", () => {
  ...
  test("radio buttons", async ({ page }, testInfo) => {
    ...
    const usingTheGridEmailForm = page.locator("nb-card", {
      hasText: "Using the Grid",
    });
    await usingTheGridEmailForm.getByLabel("Option 1").check({ force: true });
    const radioStatus = usingTheGridEmailForm
      .getByLabel("Option 1")
      .isChecked();
    ...
    expect(
      await usingTheGridEmailForm.getByLabel("Option 1").isChecked()
    ).toBeFalsy();
    expect(
      await usingTheGridEmailForm.getByLabel("Option 2").isChecked()
    ).toBeTruthy();
  });
});
...
test("web table", async ({ page }, testInfo) => {
  ...
  for (let age of ages) {
    ...
    for (let row of await ageRows.all()) {
      ...
      if (age == "200") {
        expect(await page.locator("table").textContent()).toContain(
          "No data found"
        );
...
test("sliders", async ({ page }) => {
  const locateExpenseGraph = page.locator("ngx-electricity-chart canvas");
  await locateExpenseGraph.click();
  const expenseGraph = page.locator("ngx-electricity-chart .echart div").nth(1);

  const boundingBox = await expenseGraph.boundingBox();

  // Perform a click on the new position of the element
  await page.mouse.click(
    boundingBox.x + boundingBox.width / 2,
    boundingBox.y + boundingBox.height / 2
  );
  await expect(expenseGraph).toContainText("670 kWh");
...
test("dialogs", async ({ page }, testInfo) => {
  ...
  await expect(
    page
      .locator("nb-card", { hasText: "Return Result From Dialog" })
      .getByRole("listitem")
  ).not.toBeVisible();
  await page.getByRole("button", { name: "Enter Name" }).click();
  const name = "Maayan";
...
test("parametrized methods", async ({ page }) => {
  ...
  await pm
    .onFormLayoutPage()
    .submitUSingTheGridFormWithCardenialsAndSelectOption(
      "test@test.com",
      "1020304050",
      "Option 1"
    );
...
test("register and login", async ({ page }, testInfo) => {
  const pm = new PageManager(page);
  if (testInfo.project.name == "mobile") {
    await page.locator(".sidebar-toggle").click();
  }
  ...
  await page.getByRole("textbox", { name: "Full name" }).fill(randomFullName);
  await page.getByRole("textbox", { name: "Email address" }).fill(randomEmail);
  const password = `${faker.string.alpha(5)}${faker.number.int(1000)}`;
  await page.getByRole("textbox", { name: "Password" }).first().fill(password);
  await page.locator("#input-re-password").fill(password);
  await page.locator("nb-checkbox .custom-checkbox").click();
